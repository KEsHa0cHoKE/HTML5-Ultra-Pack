...
CloseButton = function(_parId, _x, _y) constructor
{
	...
	event_step = function()
	{
		x = par_id.x+par_id.width/2 
		y = par_id.y-par_id.height/2
...
if (YG.is_release_build) {
	if (YG_MODE == E_YG_MODE.PLAYGAMA) {
		var _interState = playgama_bridge_advertisement_interstitial_state()
		var _isAdvOpened = _interState == "opened" || _interState == "loading"
		
		adv_state = _isAdvOpened ? E_ADV_STATE.SHOWING_ADV : E_ADV_STATE.CANNOT_SHOW
		
		if (!_isAdvOpened)
			alarm[0] = adv_periodicity_in_sec*game_get_speed(gamespeed_fps)
	}
	else { // YG_MODE = YANDEX_GAMES
...
{
	return (YG.adv.interstitial.is_supported && adv_state == E_ADV_STATE.CAN_SHOW && reward_state == E_REWARD_STATE.NOT_SHOW)
}

///@func met_show_inter
///@desc Показать полноэкранную рекламу
///@param {Bool} _showWarning если true - запускает предупреждение о скором показе рекламы, а через 2 секунды саму рекламу.
///@param {Function} _callback коллбек, выполнится когда игрок закроет рекламу
met_show_inter = function(_showWarning = false, _callback = undefined)
{
...
met_show_reward = function(_callBack, _callBackWithoutReward = undefined)
{
	if (!YG.adv.reward.is_supported)
		show_error("yg_adv : met_show_reward() -> Реварды не поддерживаются на площадке", true)
	
	reward_state = E_REWARD_STATE.SENDED_REQUEST
	
	req_idReward = (YG.is_release_build ? 
	(YG_MODE == E_YG_MODE.YANDEX_GAMES ? YaGames_showRewardedVideo() : playgama_bridge_advertisement_show_rewarded()) :
	__met_create_fakeReward())
